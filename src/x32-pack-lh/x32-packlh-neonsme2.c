...
// This function just wraps KleidiAI's `kai_run_lhs_pack_f32p2vlx1_f32_sme`, but
// with a name that is recognized by our tooling.
void xnn_x32_pack_lh_ukernel__neonsme(size_t m, size_t k, size_t mr,
                                          size_t kr, size_t sr,
                                          size_t m_idx_start,
                                          const float* XNN_RESTRICT lhs,
                                          size_t lhs_stride,
...
size_t xnn_x32_pack_lh_size__neonsme(size_t m, size_t k, size_t mr, size_t kr, size_t sr) {
#if XNN_ENABLE_KLEIDIAI
  return kai_get_lhs_packed_size_lhs_pack_f32p2vlx1_f32_sme(m, k, mr, kr, sr);
#else
  assert("Not compiled with XNN_ENABLE_KLEIDIAI" && 0);
...
size_t xnn_x32_pack_lh_offset__neonsme(size_t m, size_t k, size_t mr, size_t kr, size_t sr) {
#if XNN_ENABLE_KLEIDIAI
  return kai_get_lhs_packed_offset_lhs_pack_f32p2vlx1_f32_sme(m, k / sizeof(float), mr, kr, sr);
#else
  assert("Not compiled with XNN_ENABLE_KLEIDIAI" && 0);
